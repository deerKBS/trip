  ...
  <div class="card shadow" style="width: 18rem; height: 16rem">
    ...
    <div class="card-body pt-0 pb-0 box-content">
      ...
      <div class="d-flex justify-content-end justify-i">
        <i v-if="!isFavorite" type="button" class="bi bi-star custom-star" @click="favoriteManage()" />
...
export default {
  ...
  methods: {
    ...
    async likeManage() {
      try {
        ...
        this.isLike = data;
        if (this.isLike) this.likeCount = this.likeCount + 1;
  ...
  watch: {
    item: {
      immediate: true,
      handler(newItem) {
        // item이 변경될 때 isFavorite와 isLike 초기화
        this.isFavorite = newItem.isFavoriteCheck;
        this.isLike = newItem.isLikeCheck;
        this.likeCount = newItem.likeCount;
      },
    },
  },
};
